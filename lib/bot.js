...
var lastUrl = '';
var currentTorrentProvidersname = [];
...
function bot(torrent, bot) {
  ...
  bot.onText(/\/getproviders/, async msg => {
    if(authorizedChats.includes(`${msg.chat.id}`)){
      ...
      providers.forEach(provider => {
        providersName.push(provider.name);
      });
      if(providersName.length > 0){
        var result = "";
        providersName.forEach((provider, i) => {
  ...
  bot.onText(searchRegex, async (msg, match) => {
    if(authorizedChats.includes(`${msg.chat.id}`)){
        ...
        if (!data || data.error) {
          ...
          bot.sendMessage(from, `${results1} \n This Message will be Auto Deleted in 30 Secs`).then(m => {
            setTimeout(() => {
