    ...
    if(authorizedChats.includes(`${msg.chat.id}`)){
        ...
        try {
          ...
          if(searchResult.length > 0){
            ...
            if(result != ""){
              bot.sendMessage(chat, `If You Want to Download Copy this Link and Use the Appropriate Syntax for Getting Magnet Link.\n\n${result} \n\n This Search Result will be Automatically Deleted After 30 Seconds`).then(m => {
                setTimeout(() => {
                  bot.deleteMessage(chat, m.message_id);
                }, 30000);
              })
  ...
  bot.onText(/\/getmagnet (.+)/, async (msg, match) => {
    if(authorizedChats.includes(`${msg.chat.id}`)){
      ...
      try {
        ...
        var resultTor = providers.filter(provider => {
